  ...
  useEffect(()=>{
    const handleSession = async (sess) => {
      if(sess?.user){
        ...
        console.log('handleSession email:', email)
        const r = await getUserRole(email)
        console.log('handleSession role:', r)
        const found = ROLES.find(x=>x.id===r) || ROLES[0]
        localStorage.setItem('einvoice-role', found.id)
        setRole(found)
      } else {
        setSession(null)
        setRole(ROLES.find(x=>x.id===localStorage.getItem('einvoice-role')) || ROLES[0])
      }
    }
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange(async(event,sess)=>{
      ...
      // INITIAL_SESSION, SIGNED_IN, TOKEN_REFRESHED 都統一處理
