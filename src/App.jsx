...
// Tab 1：上傳 & 紀錄
// ═══════════════════════════════════════════════════════════════════════════════
function Tab1({onAddFiles,records,loadingRecords,session,onDeleteRecord}){
  ...
  return(
    <div style={{animation:'fadeUp .3s ease'}}>
      ...
      {loadingRecords?(
        ...
        <Card>
          <div style={{display:'grid',gridTemplateColumns:'28px 1fr 80px 80px 140px 80px 80px',padding:'10px 18px',background:C.bgWarm,fontSize:10,fontFamily:"'Outfit',sans-serif",color:C.textMuted,letterSpacing:'0.07em',fontWeight:600,textTransform:'uppercase',gap:8,borderBottom:`1px solid ${C.border}`}}>
            {['','檔案名稱','大小','類型','上傳時間','狀態','操作'].map((h,i)=><span key={i}>{h}</span>)}
          </div>
          {filtered.map((r,idx)=>{
            const ts=new Date(r.uploaded_at).getTime()
            return(
              <div key={r.id} style={{display:'grid',gridTemplateColumns:'28px 1fr 80px 80px 140px 80px 80px',padding:'13px 18px',alignItems:'center',gap:8,borderBottom:idx<filtered.length-1?`1px solid ${C.borderLight}`:'none',transition:'background .12s'}}
                onMouseEnter={e=>e.currentTarget.style.background=C.bgHover}
...
export default function App(){
  ...
  const addTab1=useCallback(async files=>{
    for(const f of files){
      ...
      try{
        ...
        const rec={id,name:f.name,size:f.size,uploaded_at:new Date().toISOString(),tab_type:'tab1',status:'pending',storage_path:path,user_id:session?.user?.id,uploader:session?.user?.user_metadata?.name||session?.user?.email?.split('@')[0]||role.label}
  ...
  const addTab3=useCallback(async files=>{
    for(const f of files){
      ...
      try{
        ...
        const rec={id,name:f.name,size:f.size,uploaded_at:new Date().toISOString(),tab_type:'tab3',status:'attach',storage_path:path,user_id:session?.user?.id,uploader:session?.user?.user_metadata?.name||session?.user?.email?.split('@')[0]||role.label}
        await insertRecord(rec);setRecords(p=>[rec,...p])
      }catch(e){alert('上傳失敗：'+e.message)}
    }
  },[role,session])
  ...
  const handleDeleteRecord=useCallback(async(record)=>{
    if(!window.confirm(`確定刪除 ${record.name} ?`)) return

    try{
      const bucket=
        record.tab_type==='tab3'
          ? PDF_BUCKET
          : EXCEL_BUCKET
    ...
    <>
      ...
      <div style={{minHeight:'100vh',background:C.bg,padding:'28px 24px 60px'}}>
        <div style={{maxWidth:1060,margin:'0 auto'}}>
          ...
          {/* ── 內容 ── */}
          {tab===0&&<Tab1 onAddFiles={addTab1} records={records} loadingRecords={loadingRecords} session={session} onDeleteRecord={handleDeleteRecord}/>}
