...
function RoleSelector({role}){
  ...
  return(
    <div style={{background:'#0d1525',border:`1px solid ${col}44`,borderRadius:12,padding:'9px 15px',display:'flex',alignItems:'center',gap:9}}>
      <span style={{fontSize:18}}>{role.icon}</span>
      <div>
        <div style={{fontSize:12,color:col,fontFamily:"'DM Mono',monospace",lineHeight:1}}>身份</div>
        <div style={{fontSize:13,fontWeight:700,color:'#e2e8f0',lineHeight:1.4}}>{role.label}</div>
      </div>
    </div>
  )
}
  ...
  return(
    <div style={{animation:'fadeUp .3s ease'}}>
      <div onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)}
        ...
        <input ref={ref} type="file" multiple accept=".xlsx,.xls,.xlsm" style={{display:'none'}} onChange={e=>{handleFiles([...e.target.files]);e.target.value=''}}/>
      </div>
      ...
      {loadingRecords?(
        <div style={{textAlign:'center',padding:'48px',color:'#334155',fontFamily:"'DM Mono',monospace",fontSize:13}}><Spinner/> 載入紀錄中…</div>
      ):filtered.length===0?(
...
function Tab2({records,onStatusUpdate,role,exportSeq,setExportSeq}){
  ...
  const tid=SELLERS[si].taxId,tn=SELLERS[si].name

  // ── early return 在所有 hooks 之後 ──
    ...
    <div style={{animation:'fadeUp .3s ease'}}>
      <div style={{background:'#0a1020',border:'1px solid #164e63',borderRadius:14,padding:'14px 20px',marginBottom:20}}>
        <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
          <span>🏢</span>
          <span style={{fontWeight:700,fontSize:13,color:'#94a3b8'}}>賣方</span>
          <div style={{display:'flex',gap:7,flexWrap:'wrap'}}>
            {SELLERS.map((s,i)=>(
              <button key={i} onClick={()=>setSi(i)} style={{padding:'7px 14px',borderRadius:10,cursor:'pointer',fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:12,transition:'all .15s',background:si===i?'#0c4a6e':'#0d1525',border:si===i?'1px solid #0ea5e9':'1px solid #1e293b',color:si===i?'#7dd3fc':'#475569'}}>
                ...
                {s.name}{i===0&&<span style={{marginLeft:5,fontSize:9,color:'#0ea5e9'}}>預設</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={{marginBottom:14,fontSize:11,fontFamily:"'DM Mono',monospace",color:'#334155'}}>
        今日匯出流水號：<span style={{color:'#0ea5e9',fontWeight:700}}>#{exportSeq}</span>　下次匯出 #{exportSeq+1}
      </div>
      {invRecords.length>0&&(
        <div style={{display:'flex',gap:9,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
          ...
          {okN>0&&<Btn onClick={doExportAll} bg="#1a0e3a" color="#a5b4fc" style={{padding:'8px 16px',fontSize:12}}>⬇ 全部匯出 1 個 CSV ({okN})</Btn>}
        </div>
      )}
      {invRecords.length===0?(
            ...
            return(
              <div key={rec.id} style={{display:'grid',gridTemplateColumns:'1fr 76px 76px 52px 130px 1fr',padding:'13px 18px',alignItems:'center',gap:8,borderBottom:idx<invRecords.length-1?'1px solid #0d1525':'none',background:'transparent'}}>
                ...
                <div style={{display:'flex',gap:5,justifyContent:'center',flexWrap:'wrap'}}>
                  {isLoad&&<span style={{fontSize:11,color:'#60a5fa',fontFamily:"'DM Mono',monospace"}}><Spinner/> 載入</span>}
                  {!isLoad&&(status==='pending'||status==='checking')&&<Btn onClick={()=>checkFile(rec)}>▶ 驗證</Btn>}
                  {!isLoad&&status==='error'&&<Btn onClick={()=>checkFile(rec)}>↺ 重試</Btn>}
                  {!isLoad&&entry?.invoices?.length>0&&<Btn onClick={()=>setModal({type:'preview',fileId:rec.id})} color="#64748b" bg="#0d1525">👁 明細</Btn>}
                  {!isLoad&&status==='error'&&rE>0&&<Btn onClick={()=>setModal({type:'errors',fileId:rec.id})} color="#f87171" bg="#2d0a0a">⚠ {rE}</Btn>}
                  {!isLoad&&(status==='ok'||status==='exported')&&<Btn onClick={()=>doExportSingle(rec)} color="#a5b4fc" bg="#1a0e3a">⬇ CSV</Btn>}
                </div>
              </div>
            )
          })}
      ...
      <div style={{marginBottom:32}}>
        ...
        {canUp?(
          <div onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)}
            ...
            <p style={{color:'#334155',fontSize:12,fontFamily:"'DM Mono',monospace"}}>支援 PDF、圖片及其他格式　·　儲存至雲端</p>
            ...
            {t3.map((rec,idx)=>(
              <div key={rec.id} style={{display:'grid',gridTemplateColumns:'28px 1fr 80px 150px 90px',padding:'12px 18px',alignItems:'center',gap:8,borderBottom:idx<t3.length-1?'1px solid #0d1525':'none',background:'transparent'}}>
                ...
                <div style={{display:'flex',justifyContent:'center'}}>
                  {dlLoading===rec.id?<span style={{fontSize:11,color:'#60a5fa'}}><Spinner/></span>:<Btn onClick={()=>dlFile(rec)} color="#4ade80" bg="#042b16">⬇ 下載</Btn>}
                </div>
              </div>
            ))}
          </div>
...
export default function App(){
  ...
  // 從 localStorage 讀取上次的 role，避免重新整理後閃回一般人員
  const [role,setRole]               = useState(()=>{
    const saved = localStorage.getItem('einvoice-role')
    return ROLES.find(x=>x.id===saved) || ROLES[0]
  })
  ...
  useEffect(()=>{
    const loadRole = async (email) => {
      // 直接用 fetch 繞過 Supabase client，確保帶入正確的 auth header
      try {
        const sess = (await supabase.auth.getSession()).data.session
        if (!sess) return 'staff'
        const res = await fetch(
          `https://tstcskyvyjwbybvvxaxd.supabase.co/rest/v1/user_roles?select=role&email=eq.${encodeURIComponent(email)}&limit=1`,
          {
            headers: {
              'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRzdGNza3l2eWp3YnlidnZ4YXhkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkzMzc4NzgsImV4cCI6MjA5NDkxMzg3OH0.kgdDE9iZ6sp6NWD8Rn21n6METZkUXCWt7E03GM-pZ-4',
              'Authorization': `Bearer ${sess.access_token}`,
            }
          }
        )
        const data = await res.json()
        console.log('loadRole result:', data)
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange(async(event,sess)=>{
      ...
      if(sess?.user){
        ...
        console.log('loading role for:', email)
        const r = await loadRole(email)
        console.log('role result:', r)
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      ...
      try{
        ...
        setRecords(recs);setExportSeq(seq)
  ...
  // 若 5 秒後還沒有 session 事件，強制設為 null（避免永久 loading）
  useEffect(()=>{
  ...
  if(session===undefined) return(
    <div style={{minHeight:'100vh',background:'#060b16',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <span style={{color:'#334155',fontFamily:"'DM Mono',monospace",fontSize:13}}><Spinner/> 載入中…</span>
    </div>
  )
  ...
  return(
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}body{background:#060b16;color:#e2e8f0}
        ...
        <div style={{maxWidth:1040,margin:'0 auto'}}>
          <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:26,flexWrap:'wrap'}}>
            ...
            <div>
              ...
              <p style={{color:'#334155',fontSize:11,fontFamily:"'DM Mono',monospace",marginTop:3}}>ERP 銷貨單 Excel → 財政部申報 CSV　·　☁ 雲端儲存</p>
            </div>
            <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
              ...
              <div style={{background:'#0d1525',border:'1px solid #1e293b',borderRadius:12,padding:'8px 14px'}}>
                <div style={{fontSize:12,fontWeight:700}}>{session.user?.user_metadata?.name||session.user?.email?.split('@')[0]}</div>
                <div style={{fontSize:10,color:'#475569',fontFamily:"'DM Mono',monospace"}}>{session.user?.email}</div>
              </div>
              <RoleSelector role={role}/>
              {role.id==='admin'&&(
                <button onClick={()=>setShowAdmin(true)} style={{background:'#1a0e3a',border:'1px solid #6366f1',color:'#a5b4fc',borderRadius:10,padding:'8px 14px',cursor:'pointer',fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:12}}>🛡 權限管理</button>
              )}
              <button onClick={async()=>{
                  localStorage.removeItem('einvoice-role')
                  await supabase.auth.signOut()
                  window.location.replace(window.location.origin)
                }} style={{background:'#0d1525',border:'1px solid #334155',color:'#64748b',borderRadius:10,padding:'8px 14px',cursor:'pointer',fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:12}}
                onMouseEnter={e=>{e.currentTarget.style.borderColor='#f87171';e.currentTarget.style.color='#f87171'}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor='#334155';e.currentTarget.style.color='#64748b'}}>登出</button>
            </div>
          </div>
