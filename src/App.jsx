...
function Tab2({records,onStatusUpdate,role,exportSeq,setExportSeq}){
  ...
  const doExportAll=useCallback(async()=>{
    ...
    const allEntries=await Promise.all(passed.map(r=>loadFile(r, tid)))
    ...
    <div style={{animation:'fadeUp .3s ease'}}>
      ...
      {invRecords.length>0&&(
        <div style={{display:'flex',gap:10,marginBottom:18,flexWrap:'wrap',alignItems:'center'}}>
          ...
          <div style={{flex:1}}/>
          {pN>0&&<Btn onClick={()=>invRecords.filter(r=>getStatus(r)==='pending').forEach(r=>checkFile(r, SELLERS[si].taxId))} style={{padding:'9px 18px',fontSize:12}}>▶ 全部驗證 ({pN})</Btn>}
            ...
            return(
              <div key={rec.id} style={{display:'grid',gridTemplateColumns:'1fr 76px 76px 52px 120px 1fr',padding:'14px 18px',alignItems:'center',gap:8,borderBottom:idx<invRecords.length-1?`1px solid ${C.borderLight}`:'none',transition:'background .12s'}}
                ...
                <div style={{display:'flex',gap:5,justifyContent:'center',flexWrap:'wrap'}}>
                  ...
                  {!isLoad&&(status==='pending'||status==='checking')&&<Btn onClick={()=>checkFile(rec, SELLERS[si].taxId)}>▶ 驗證</Btn>}
                  {!isLoad&&status==='error'&&<Btn onClick={()=>checkFile(rec, SELLERS[si].taxId)} variant="ghost">↺ 重試</Btn>}
